...
function LoadCryptoCoins(){
    $.post('https://Renewed-Phone/GetCryptosFromDegens', JSON.stringify({}), function(Jobs){
        $(".crypto-lists").html("");
...
$(document).on('click', '#crypto-send-purchase', function(e){
    ...
    if(amount != ""){
        ...
        $.post('https://Renewed-Phone/BuyCrypto', JSON.stringify({
            metadata: crypto,
            amount: amount,
        }));
    }
...
$(document).on('click', '#crypto-send-exchange', function(e){
    ...
    if(amount != "" || stateid != ""){
        if (CryptoType[crypto] - amount >= 0){
            ...
            $.post('https://Renewed-Phone/ExchangeCrypto', JSON.stringify({
                metadata: crypto,
                amount: amount,
                stateid: stateid,
            }));
...
$(document).on('click', '#crypto-send-sell', function(e){
    ...
    if(amount != ""){
        if (CryptoType[crypto] - amount >= 0){
            ...
            $.post('https://Renewed-Phone/SellCrypto', JSON.stringify({
                metadata: crypto,
                amount: amount,
            }));
        }else{
