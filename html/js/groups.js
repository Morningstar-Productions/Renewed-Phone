...
function LoadJobCenterApp(){
    $.post('https://Renewed-Phone/GetGroupsApp', JSON.stringify({}), function(data){
        AddDIV(data)
    });
}
...
$(document).on('click', '#jobcenter-submit-create-group', function(e){
    ...
    if (Name != "" && pass != "" && pass2 != ""){
        if(pass == pass2){
            $.post('https://Renewed-Phone/jobcenter_CreateJobGroup', JSON.stringify({
                name: Name,
                pass: pass,
            }));
...
$(document).on('click', '#jobcenter-delete-group', function(e){
    ...
    $.post('https://Renewed-Phone/jobcenter_DeleteGroup', JSON.stringify({
        delete: Delete,
    }));
});
...
$(document).on('click', '#jobcenter-submit-join-group', function(e){
    ...
    if(EnterPass == JoinPass){
        ...
        $.post('https://Renewed-Phone/jobcenter_JoinTheGroup', JSON.stringify({
            PCSN: CSN,
            id: JoinID,
        }));
        ClearInputNew()
...
$(document).on('click', '#jobcenter-list-group', function(e){
    e.preventDefault();
    var id = $(this).data('id')
    $.post('https://Renewed-Phone/jobcenter_CheckPlayerNames', JSON.stringify({
        id: id,
        }), function(Data){
           ClearInputNew()
...
$(document).on('click', '#jobcenter-leave-grouped', function(e){
    ...
    var CSN = QB.Phone.Data.PlayerData.citizenid;
    var id = $(this).data('id')
    $.post('https://Renewed-Phone/jobcenter_leave_grouped', JSON.stringify({
        id: id,
        csn: CSN,
    }));
});
