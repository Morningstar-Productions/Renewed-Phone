...
$(document).on('click', '.employment-list', function(e){
    ...
    $.post('https://Renewed-Phone/GetEmployees', JSON.stringify({job: job}), function(data){
...
$(document).on('click', '#employment-job-extras-icon', function(e){
    ...
    $.post('https://Renewed-Phone/dutyStatus', JSON.stringify({}), function(data) {
        currentJob = data["job"]
        if (data["duty"]) {
            onDuty = true
        } else {
...
$(document).on('click', '#clock-in', function(e){
    ...
    $.post('https://Renewed-Phone/ClockIn', JSON.stringify({
        job: job
    }));
    closeDropDown()
});
...
$(document).on('click', '#hire-worker-submit', function(e){
    ...
    if(stateid !== "" && grade !== ""){
        ...
        $.post('https://Renewed-Phone/HireFucker', JSON.stringify({
            stateid: stateid,
            grade: grade,
            job: job,
        }));
...
$(document).on('click', '#employment-chargemf-submit', function(e){
    ...
    if(stateid != "" && amount != "" && note != ""){
        ...
        $.post('https://Renewed-Phone/ChargeMF', JSON.stringify({
            stateid: stateid,
            amount: amount,
            note: note,
            job: job,
...
$(document).on('click', '#send-employee-payment', function(e){
    ...
    if(amount != "" && note != ""){
        ...
        $.post('https://Renewed-Phone/SendEmployeePayment', JSON.stringify({
            cid: cid,
            job: job,
            amount: amount,
            note: note,
...
$(document).on('click', '#employment-remove-employee', function(e){
    ...
    $.post('https://Renewed-Phone/RemoveEmployee', JSON.stringify({
        cid: cid,
        job: job,
    }));
});
...
$(document).on('click', '#employment-changerole-submit', function(e){
    ...
    if(grade != ""){
        ...
        $.post('https://Renewed-Phone/ChangeRole', JSON.stringify({
            cid: cid,
            grade: grade,
            job: job
        }));
